...
dl_ua3f() {
    ...
    wget https://cdn.jsdelivr.net/gh/SunBK201/UA3F@master/openwrt/files/ua3f.uci
    if [ $? -ne 0 ]; then
        echo "Download ua3f.uci Failed, Please Retry."
        exit 1
    fi

    wget https://cdn.jsdelivr.net/gh/SunBK201/UA3F@master/openwrt/files/luci/cbi.lua
    if [ $? -ne 0 ]; then
        echo "Download cbi.lua Failed, Please Retry."
        exit 1
    fi

    wget https://cdn.jsdelivr.net/gh/SunBK201/UA3F@master/openwrt/files/luci/controller.lua
    if [ $? -ne 0 ]; then
